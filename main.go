...
var dmatch mux.MatcherFunc = func(request *http.Request, match *mux.RouteMatch) bool {
	...
	return regexp.MustCompile(`^(\/api\/(tenants|sites|buildings|rooms|acs|panels|separators|cabinets|aisles|tiles|groups|corridors|racks|devices|sensors|(room|obj)-templates)\?.*)$`).
...
func main() {
	...
	router.HandleFunc("/api/buildings/{id:[a-zA-Z0-9]{24}}/{sub:acs|corridors|cabinets|tiles|aisles|panels|separators|sensors|groups}",
		controllers.GetEntitiesOfAncestor).Methods("GET")
