...
var dmatch mux.MatcherFunc = func(request *http.Request, match *mux.RouteMatch) bool {
	...
	return regexp.MustCompile(`^(\/api\/(tenants|sites|buildings|rooms|acs|panels|separators|cabinets|aisles|tiles|groups|corridors|racks|devices|rack-sensors|device-sensors|(room|obj)-templates)\?.*)$`).
...
var hmatch mux.MatcherFunc = func(request *http.Request, match *mux.RouteMatch) bool {
	...
	return regexp.MustCompile(`(^(\/api\/(tenants|sites|buildings|rooms|rooms|racks|devices)\/[a-zA-Z0-9]{24}\/all)(\/(tenants|sites|buildings|rooms|rooms|racks|devices))*$)|(^(\/api\/(tenants|sites|buildings|rooms|rooms|racks|devices)\/[a-zA-Z0-9]{24}\/all)(\?limit=[0-9]+)*$)`).
		MatchString(request.URL.String())
}
...
func main() {
	...
	router.HandleFunc("/api/buildings/{id:[a-zA-Z0-9]{24}}/{sub:acs|corridors|cabinets|tiles|aisles|panels|separators|room-sensors}",
		controllers.GetEntitiesOfAncestor).Methods("GET")
