...
// swagger:operation GET /api/user/sites/{id}/all/buildings/rooms/racks/devices sites GetSiteHierarchy
// Gets Hierarchy of a Site until devices.
// ---
// produces:
// - application/json
// parameters:
...
//     '404':
//         description: Not Found
var GetSiteHierarchyToDevice = func(w http.ResponseWriter, r *http.Request) {
	id, e := strconv.Atoi(mux.Vars(r)["id"])
	resp := u.Message(true, "success")
	if e != nil {
		u.Respond(w, u.Message(false, "Error while parsing path parameters"))
		u.ErrLog("Error while parsing path parameters", "GET SITEHIERARCHYTODEVICE", "", r)
		return
	}

	data, e1 := models.GetSiteHierarchyToDevice(id)
	if data == nil {
		resp = u.Message(false, "Error while getting Site Hierarchy: "+e1)
		u.ErrLog("Error while getting Site Hierarchy",
			"GET SITEHIERARCHYTODEVICE", e1, r)
...
// swagger:operation GET /api/user/sites/{id}/buildings/{building_name}/rooms/{room_name}/racks/{rack_name}/devices/{device_name}/subdevices/{subdevice_name}/subdevice1s sites GetDevicesOfSite
// Gets Subdevices of a Named Subdevice from Site.
// ---
// produces:
// - application/json
// parameters:
...
//     '404':
//         description: Not Found
var GetSubdevice1sUsingNamedSubdeviceOfSite = func(w http.ResponseWriter, r *http.Request) {
	id, e := strconv.Atoi(mux.Vars(r)["id"])
	bldg_name := mux.Vars(r)["building_name"]
	room_name := mux.Vars(r)["room_name"]
	rack_name := mux.Vars(r)["rack_name"]
	...
	if e != nil {
		u.Respond(w, u.Message(false, "Error while parsing path parameters"))
		u.ErrLog("Error while parsing path parameters", "GET SUBDEV1SUSINGNAMEDSUBDEVICEOFSITE", "", r)
		return
	}

	data, e1 := models.GetSubdevice1sUsingNamedSubdeviceOfSite(id, bldg_name,
		room_name, rack_name, dev_name, subdev_name)
	if data == nil || len(data) == 0 {
		resp = u.Message(false, "Error while getting Subdevices: "+e1)
		u.ErrLog("Error while getting Subdevices Using Named Device Of Site",
			"GET SUBDEV1SUSINGNAMEDSUBDEVICEOFSITE", e1, r)
...
// swagger:operation GET /api/user/sites/{id}/buildings/{building_name}/rooms/{room_name}/racks/{rack_name}/devices/{device_name}/subdevices/{subdevice_name}/subdevice1s/{subdevice1_name} sites GetDevicesOfSite
// Gets a Named Subdevice of a Site.
// ---
// produces:
// - application/json
...
//     '404':
//         description: Not Found
var GetNamedSubdevice1OfSite = func(w http.ResponseWriter, r *http.Request) {
	id, e := strconv.Atoi(mux.Vars(r)["id"])
	bldg_name := mux.Vars(r)["building_name"]
	room_name := mux.Vars(r)["room_name"]
	rack_name := mux.Vars(r)["rack_name"]
	device_name := mux.Vars(r)["device_name"]
	subdevice_name := mux.Vars(r)["subdevice_name"]
	subdevice1_name := mux.Vars(r)["subdevice1_name"]
	resp := u.Message(true, "success")
	if e != nil {
		u.Respond(w, u.Message(false, "Error while parsing path parameters"))
		u.ErrLog("Error while parsing path parameters", "GET NAMEDSUBDEV1OFSITE", "", r)
		return
	}

	data, e1 := models.GetNamedSubdevice1OfSite(id, bldg_name, room_name,
		rack_name, device_name, subdevice_name, subdevice1_name)
	if data == nil {
		resp = u.Message(false, "Error while getting Subdevice1: "+e1)
		u.ErrLog("Error while getting Named Subdevice1 Of Site",
			"GET NAMEDSUBDEV1OFSITE", e1, r)
