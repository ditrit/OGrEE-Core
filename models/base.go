...
func init() {
	...
	db.Model(&Tenant_Attributes{}).AddForeignKey(
		"tenant_id", "tenant(id)", "CASCADE", "CASCADE")
	...
	db.Model(&Rack{}).AddForeignKey("domain", "rooms(id)", "CASCADE", "CASCADE")
	db.Model(&Device{}).AddForeignKey("domain", "racks(id)", "CASCADE", "CASCADE")
	println("GOT HERE 3")
}
