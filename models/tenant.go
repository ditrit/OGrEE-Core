...
func GetTenantHierarchyToBuilding(tenant_name string) (*Tenant, string) {
	tenant, e := GetTenantByName(tenant_name)
	if e != "" {
		return nil, e
	}
...
func GetTenantHierarchyToRoom(tenant_name string) (*Tenant, string) {
	tenant, e := GetTenantByName(tenant_name)
	if e != "" {
		return nil, e
	}
...
func GetTenantHierarchyToSubdevice(tenant_name string) (*Tenant, string) {
	tenant, e := GetTenantByName(tenant_name)
	if e != "" {
		return nil, e
	}
		...
		for k, _ := range tenant.Sites[idx].Buildings {
			...
			for i, _ := range tenant.Sites[idx].Buildings[k].Rooms {
				...
				for j, _ := range tenant.
					...
					for q, _ := range tenant.Sites[idx].Buildings[k].Rooms[i].
						Racks[j].Devices {
						tenant.Sites[idx].Buildings[k].Rooms[i].
							Racks[j].Devices[q].Subdevices, e = GetSubdevicesOfParent(
							uint(tenant.Sites[idx].Buildings[k].Rooms[i].
								Racks[j].Devices[q].ID))
						if e != "" {
							return nil, e
						}
					}
...
func GetNamedSubdevice1OfTenant(tenant_name, site_name,
	bldg_name, room_name, rack_name, dev_name,
	...
	return GetSubdevice1ByNameAndParentID(subdev.ID, subdev1_name)
}
