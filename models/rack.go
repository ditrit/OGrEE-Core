...
func GetRackHierarchyToSubdevices(id int) (*Rack, string) {
	...
	for i, _ := range rack.Devices {
		rack.Devices[i].Subdevices, e =
			GetSubdevicesOfParent(uint(rack.Devices[i].ID))
	}
	return rack, ""
}
...
func GetSubdevice1sUsingNamedSubdeviceOfRack(id int, dev, subdev string) ([]*Subdevice1, string) {
	device, e := GetDeviceByNameAndParentID(uint(id), dev)
	if e != "" {
		return nil, ""
	}
	...
	sd1s, e2 := GetSubdevices1OfParent(subdevice.ID)
	if e2 != "" {
		return nil, ""
	}
	return sd1s, ""
