...
func GetEntityHierarchy(ID primitive.ObjectID, req bson.M, ent string, start, end int, filters []string) (map[string]interface{}, string) {
	var childEnt string
	...
	if start < end {
		...
		top, e := GetEntity(newReq, ent, filters)
		if top == nil {
			return nil, e
		}
		top = fixID(top)
		...
		for idx := range subEnts {
			tmp, _ := GetEntityHierarchy(subEnts[idx]["id"].(primitive.ObjectID), req, childEnt, start+1, end, filters)
			if tmp != nil {
				subEnts[idx] = tmp
			}
		}
...
func GetEntitiesUsingAncestorNames(ent string, id primitive.ObjectID, req map[string]interface{}, ancestry []map[string]string) ([]map[string]interface{}, string) {
	...
	top, e := GetEntity(newReq, ent, []string{})
	if e != "" {
		return nil, e
	}
...
func GetHierarchyByName(entity, name string, req bson.M, entnum, end int) (map[string]interface{}, string) {
	...
	t, e := GetEntity(newReq, entity, []string{})
	if e != "" {
		fmt.Println(e)
		return nil, e
	}
	...
	for i := range children {
		...
		x, _ =
			GetEntityHierarchy(subID, subReq, subIdx, entnum+1, end, []string{})
		if x != nil {
			children[i] = x
		}
	}
...
func GetEntitiesOfAncestor(id interface{}, req bson.M, ent int, entStr, wantedEnt string) ([]map[string]interface{}, string) {
	...
	if ent == SITE {
		...
		t, e = GetEntity(newReq, "site", []string{})
		if e != "" {
			return nil, e
		}
		...
		t, e = GetEntity(newReq, entStr, []string{})
		if e != "" {
			return nil, e
		}
	}
...
func DeleteDeviceF(entityID primitive.ObjectID, req bson.M) (map[string]interface{}, string) {
	...
	t, e := GetEntityHierarchy(entityID, req, "device", 0, 999, []string{})
	if e != "" {
		return u.Message(false,
