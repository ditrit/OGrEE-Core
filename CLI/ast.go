...
func (n *getObjectNode) execute() (interface{}, error) {
	...
	objs, _, err := cmd.Get.GetObjectsWildcard(path)
	if err != nil {
		return nil, err
	}
...
func (n *selectObjectNode) execute() (interface{}, error) {
	...
	if strings.Contains(path, "*") {
		_, selection, err = cmd.Get.GetObjectsWildcard(path)
		if err != nil {
			return nil, err
		}
	} else if path != "" {
...
func (n *updateObjNode) execute() (interface{}, error) {
	...
	if path == "_" {
		...
		_, paths, err = cmd.Get.GetObjectsWildcard(path)
		if err != nil {
			return nil, err
		}
	} else {
...
func (n *drawNode) execute() (interface{}, error) {
	...
	if strings.Contains(path, "*") {
		_, subpaths, err := cmd.Get.GetObjectsWildcard(path)
		if err != nil {
			return nil, err
		}
