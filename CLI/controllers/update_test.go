...
func TestUpdateSlugOfEntityDifferentToTagReturnsError(t *testing.T) {
	_, err := controllers.Update.UpdateObj(models.BuildingTemplatesPath+"old-slug", map[string]any{
		"slug": "new-slug",
	})
	assert.NotNil(t, err)
	assert.ErrorContains(t, err, "slug can only be edited on tags")
}
