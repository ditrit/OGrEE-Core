...
func getHierarchyWithNamespace(namespace u.Namespace, userRoles map[string]Role, filters u.HierarchyFilters,
	...
	for _, entity := range entities {
		...
		data, e := ExtractCursor(c, ctx, entity, userRoles)
		if e != nil {
...
func GetStats() map[string]interface{} {
	...
	for entityType := range u.EntityTypeList {
		num := GetEntityCount(entityType)
		if num == -1 {
			num = 0
		}

		ans["Number of "+u.EntityToString(entityType)+"s:"] = num
		...
		e := repository.GetDB().Collection(u.EntityToString(entityType)).FindOne(ctx, bson.M{}, filter).Decode(&obj)
		if e == nil {
...
func UpdateEntity(ent string, id string, updateData map[string]interface{}, isPatch bool, userRoles map[string]Role) (map[string]interface{}, *u.Error) {
	...
	result, err := WithTransaction(func(ctx mongo.SessionContext) (interface{}, error) {
		...
		if entInt == u.TAG && updateData["slug"].(string) != oldObj["slug"].(string) {
